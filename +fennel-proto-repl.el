  ...
  (let ((id (plist-get message :id))
    ...
    (when-let ((callbacks (fennel-proto-repl--get-callbacks id)))
      (pcase op
        ...
        ("print"
         (progn
           (let ((print-cb (if (= id -1)
                               #'fennel-proto-repl--print
                             (fennel-proto-repl-callback-print callbacks))))
             ;; (message "running print callback %s for message %s ..." print-cb message)
             (funcall print-cb
    ...
    (save-excursion
      (save-restriction
        (let ((inhibit-read-only t)
              ...
              (prompt-start (if (comint-check-proc (current-buffer))
                                (progn
                                  (comint-goto-process-mark)
                                  (forward-char -1)
                                  (field-beginning))
